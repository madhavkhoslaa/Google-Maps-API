...
class API():
    def __init__(self, key, AreaLatLongBound, MapZoom, ImageSize):
        ...
        self.query_pts = self.createQueryFragments(
            self.AreaLatLongBound[0],
            self.AreaLatLongBound[1],
            self.AreaLatLongBound[2],
            self.AreaLatLongBound[3])
        self.faddr = []
        self.geo = []
        self.centroids = []
    ...
    def createQueryFragments(
            self, topLeftLat, topLeftLong, bottomRightLat, bottomRightLong):
        down = 0.005900
        right = 0.006600
        mar = 0.0002
        return np.mgrid[topLeftLat - (down / 2):bottomRightLat:-0.005700, topLeftLong + (
            right / 2):bottomRightLong:0.006400].reshape(2, -1).T

    def white_image(self, im):
        # returns a white image of the shape input image
        return cv2.bitwise_not(np.zeros(im.shape, np.uint8))

    def reversegeocode(self, lat, long_, radius):
        base = "https://maps.googleapis.com/maps/api/geocode/json?latlng={},{}&radius={}&key={}".format(
            str(lat), str(long_), str(radius),self.key)
    ...
    def distance_lat_long(self, point1, point2):
        ...
        a = math.sin(
            dlat / 2)**2 + math.cos(point1[0]) * math.cos(point2[0]) * math.sin(dlon / 2)**2
    ...
    def getboundries(self, center_latitude, center_longitude, save=True):
        ...
        urlBuildings = "http://maps.googleapis.com/maps/api/staticmap?center=" + str_Center + "&zoom=" + self.MapZoom + \
            "&format=png32&sensor=false&size=" + self.ImageSize + \
            "&maptype=roadmap&style=visibility:off&style=" + \
            safeURL_Style + '&key=' + str(self.key)
        ...
        if save:
            skimage.io.imsave(
                "CentreLatLong:{}MapZoom:{}.png".format(
                    str_Center,
                    self.MapZoom),
                boundry_image_raw)
        else:
            pass
        self.clat_clong_contours.update(
            {(center_latitude, center_longitude): contoursBuildings})
        return boundry_image_raw

    def getImage(self, clat, clong, save=True):
        base = "https://maps.googleapis.com/maps/api/staticmap?center={},+{}6&zoom={}&scale=1&size={}&maptype=satellite&format=png&visual_refresh=true&key={}".format(
            clat, clong, self.MapZoom, self.ImageSize, self.key)
